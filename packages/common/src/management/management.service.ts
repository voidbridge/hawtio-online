      ...
      openShiftService.on('changed', _ => {
        ...
        openShiftService.getPods().forEach(pod => {
          ...
          if (!mPod) {
            ...
            const port = 8778;
        console.log("management: " + JSON.stringify(pod));
            const url = new URI().query('').path(`/master/api/v1/namespaces/${pod.metadata.namespace}/pods/https:${pod.metadata.name}:${port}/proxy/jolokia1/`)
